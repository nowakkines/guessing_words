...
def play(tries):
    ...
    for _ in track(range(50), description='[green]Processing...'):
        sleep(0.03)
    print(
        f'[red]The word was generated[/red]: {word_completion} REMAINED {word_count} LETTERS {result} ')
    gaming(result, guessed_letters, guessed_word,
           word_as_lst, word_completion, tries)
...
def again():
    ...
    match question:
        ...
        case '-':
            console.print(
                '[blue]Thank you for playing the game...', justify='center')
            STOP_GAME = True
        case _:
            console.print(Panel('You have entered an invalid character.',
                          title='[red]Error'), justify='center')
            again(STOP_GAME)
...
def gaming(result, guessed_letters, guessed_word, word_as_lst, word_completion, tries):
    while STOP_GAME != True and word_completion != result:
        letter = input('Enter a letter or a word >> ').upper()
        ...
        if letter == 'INFO':
            ...
            if tries == 0:
                ...
                print('[red]You entered the letter that was used.')

            elif len(letter) == 1 and letter not in result:
                console.print(
                    'You entered the wrong letter - [red]your attempt is exhausted[/red].')
                tries -= 1

            elif letter == result:
                console.print(Panel(
                    'You guessed the word, you\'re a smart guy', title='[blue]Congratulations'))
                again()

            elif len(letter) == len(result) and letter in guessed_word:
                console.print(
                    '[red]Have you already entered this word and are you going to use it again?')
                show_info(guessed_letters, guessed_word)
                continue
                ...
                print('The written word was wrong. So you take away your attempt')
                guessed_word.append(letter)
                tries -= 1
        else:
            console.print(Panel(
                'You have entered an invalid character or your length does not match.', title='[red]Error'))
    else:
...
def show_info(guessed_letters, guessed_word):
    ...
    match info:
        ...
        case _:
            console.print(
                Panel('You have entered an invalid character.', title='[red]Error'))
